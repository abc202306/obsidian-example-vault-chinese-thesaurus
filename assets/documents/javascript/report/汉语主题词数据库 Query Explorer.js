...
function view(file){
    cachedResultDiv = noteCon;
    const newDiv = document.createElement("div");
    cachedResultDiv.replaceWith(newDiv);

	const linkText = app.metadataCache.fileToLinktext(file);
	const content = "![["+linkText+"]]";
	console.log(content);
	dv.paragraph(content,{container:newDiv});
	
	let page = dv.page(file.path);
    const button = document.createElement("button");
    button.innerText = "Back to Search Results";
    button.onclick = ()=>{
        newDiv.replaceWith(cachedResultDiv);
    }
    newDiv.prepend(button);
	viewHR(page,newDiv);
	showMindMap(page,newDiv);
...
function getNodeRowArr(node,node2,relations,depth=0){
	...
	if (!node.isRoot){
		...
		const curRelationsStr = curRelations.map(r=>r[0][0].toUpperCase()).unique().join("").replaceAll("U","D:").replaceAll("C","S:").replaceAll("B","N:");
