...
- `src/` | 存储相关的代码模块
	- `descriptor/` | 存储与主题词表相关的代码模块
		- `dvmodule/` | 以异步函数 `importFrom` 的导出方式为导出方式的模块（其中 `importFrom` 是 `async function importFrom(modulePath) {return new Promise(resolve=>dv.view(modulePath, resolve));}`）
			- [get-group-key.js](<../src/dvmodule/get-group-key.js>) | 导出函数 `getGroupKey(p)`
			- [get-dc-descriptors.js](<../src/dvmodule/get-dc-descriptors.js>) | 导出函数 `getDCDescriptor(p, descriptorPath)`
		- `dvscript` | 存储脚本
...
- 主题词笔记数据的来源
	- 《汉语主题词表》数据来源于 [汉语主题词表服务系统](https://ct.istic.ac.cn/site/organize/index)
